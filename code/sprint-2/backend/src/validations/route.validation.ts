...
export const createRouteByAdminSchema = createRouteSchema.extend({
  driverId: z.string().cuid({ message: 'driverId must be a CUID' }),
});

export const updateRouteByAdminSchema = updateRouteSchema.extend({
  driverId: z.string().cuid({ message: 'driverId must be a CUID' }).optional(),
});

export const adminDriverIdParamSchema = z.object({
